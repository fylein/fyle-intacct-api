...
def schedule_or_delete_fyle_import_tasks(configuration: Configuration, mapping_setting_instance: MappingSetting = None) -> None:
    """
    ...
    if (
        task_to_be_scheduled or configuration.import_categories
        or configuration.import_tax_codes or configuration.import_vendors_as_merchants
    ):
    ...
    if (
        import_fields_count == 0 and custom_field_import_fields_count == 0
