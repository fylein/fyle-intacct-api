...
def resolve_errors_for_exported_expense_group(expense_group: ExpenseGroup) -> None:
    ...
    :param expense_group: Expense group
    """
    Error.objects.filter(workspace_id=expense_group.workspace_id, expense_group=expense_group, is_resolved=False).update(is_resolved=True, updated_at=datetime.now())
...
def check_expenses_reimbursement_status(expenses: list[Expense], workspace_id: int, platform: PlatformConnector, filter_credit_expenses: bool) -> bool:
    ...
    if is_paid:
        Expense.objects.filter(workspace_id=workspace_id, report_id=report_id, paid_on_fyle=False).update(paid_on_fyle=True, updated_at=datetime.now())
...
def mark_paid_on_fyle(platform: PlatformConnector, payloads:dict, reports_to_be_marked: list, workspace_id: int, retry_num: int = 10) -> None:
    ...
    try:
        ...
        Expense.objects.filter(report_id__in=list(reports_to_be_marked), workspace_id=workspace_id, paid_on_fyle=False).update(paid_on_fyle=True, updated_at=datetime.now())
    except Exception as e:
        error = traceback.format_exc()
        ...
        for item in error_response.get('data', []):
            if item.get('message') in target_messages:
                Expense.objects.filter(report_id=item['key'], workspace_id=workspace_id, paid_on_fyle=False).update(paid_on_fyle=True, updated_at=datetime.now())
