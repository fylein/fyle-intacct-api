...
def process_expense_group_for_fund_source_update(expense_group: ExpenseGroup, changed_expense_ids: list[int], workspace_id: int, report_id: str, affected_fund_source_expense_ids: dict[str, list[int]]) -> None:
    ...
    if task_log:
        ...
        if task_log.status in ['ENQUEUED', 'IN_PROGRESS']:
            schedule_task_for_expense_group_fund_source_change(expense_group_id=expense_group.id, changed_expense_ids=changed_expense_ids, workspace_id=workspace_id, report_id=report_id, affected_fund_source_expense_ids=affected_fund_source_expense_ids)
            return False
