...
@pytest.mark.integration
class TestJournalEntriesPostWithAllocation(BaseTestPost):
    """
    Test journal entries POST operations.
    """
    REST_MODULE_NAME = 'journal_entries'
    ...
    REST_FIELDS = ['glJournal.id', 'description']
    SOAP_FIELDS = ['JOURNAL', 'BATCH_TITLE']
    REST_PAYLOAD = REST_JOURNAL_ENTRY_CREATE_PAYLOAD_WITH_ALLOCATION
    SOAP_PAYLOAD = SOAP_JOURNAL_ENTRY_CREATE_PAYLOAD_WITH_ALLOCATION
    KEY_MAPPINGS = [
        {
            'rest': 'glJournal.id',
            'soap': 'JOURNAL'
