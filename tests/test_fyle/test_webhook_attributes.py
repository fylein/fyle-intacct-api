...
def test_process_webhook_category_created(db):
    ...
    webhook_body = data['webhook_payloads']['category_created']
...
def test_process_webhook_category_updated(db):
    ...
    webhook_body = data['webhook_payloads']['category_updated']
    processor.process_webhook(webhook_body)
    expense_attr = ExpenseAttribute.objects.filter(
        workspace_id=workspace_id,
        attribute_type='CATEGORY',
...
def test_process_webhook_category_deleted(db):
    ...
    webhook_body = data['webhook_payloads']['category_deleted']
    processor.process_webhook(webhook_body)
...
def test_process_webhook_project(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    webhook_body = data['webhook_payloads']['project_created']
    processor.process_webhook(webhook_body)
    expense_attr = ExpenseAttribute.objects.filter(
        workspace_id=workspace_id,
...
def test_process_webhook_cost_center(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    webhook_body = data['webhook_payloads']['cost_center_created']
    processor.process_webhook(webhook_body)
    expense_attr = ExpenseAttribute.objects.filter(
        workspace_id=workspace_id,
    ...
    assert expense_attr is not None
    assert expense_attr.value == 'Operations'
    assert expense_attr.active is True
...
def test_process_webhook_employee(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    webhook_body = data['webhook_payloads']['employee_created']
    processor.process_webhook(webhook_body)
    expense_attr = ExpenseAttribute.objects.filter(
        workspace_id=workspace_id,
...
def test_process_webhook_corporate_card(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    webhook_body = data['webhook_payloads']['corporate_card_created']
    processor.process_webhook(webhook_body)
    expense_attr = ExpenseAttribute.objects.filter(
        workspace_id=workspace_id,
...
def test_process_webhook_tax_group(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    webhook_body = data['webhook_payloads']['tax_group_created']
    processor.process_webhook(webhook_body)
    expense_attr = ExpenseAttribute.objects.filter(
        workspace_id=workspace_id,
...
def test_process_expense_field_select_type(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    webhook_body = data['webhook_payloads']['expense_field_select_created']
...
def test_process_expense_field_disable_options(db):
    ...
    webhook_body = data['webhook_payloads']['expense_field_region_updated']
    processor.process_webhook(webhook_body)
...
def test_process_expense_field_non_select_type(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    webhook_body = data['webhook_payloads']['expense_field_text_created']
    initial_count = ExpenseAttribute.objects.filter(
        workspace_id=workspace_id,
...
def test_process_webhook_skip_when_import_in_progress(db):
    ...
    webhook_body = data['webhook_payloads']['category_skip']
    initial_count = ExpenseAttribute.objects.filter(
        workspace_id=workspace_id,
        attribute_type='CATEGORY'
    ).count()
...
def test_process_webhook_deleted_ignores_import_in_progress(db):
    ...
    webhook_body = data['webhook_payloads']['cost_center_deleted']
    processor.process_webhook(webhook_body)
    expense_attr = ExpenseAttribute.objects.get(
        workspace_id=workspace_id,
        source_id='cc_delete'
...
def test_get_nested_value(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    test_data = data['webhook_test_data']['nested_value_data']
    assert processor._get_nested_value(test_data, 'user.email') == 'test@example.com'
    assert processor._get_nested_value(test_data, 'user.profile.department.name') == 'Engineering'
    assert processor._get_nested_value(test_data, 'user.profile.department.code') == 'ENG'
    assert processor._get_nested_value(test_data, 'user.nonexistent.path') == {}
    assert processor._get_nested_value(test_data, 'completely.wrong.path') == {}
...
def test_get_attribute_data_category(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    test_data = data['webhook_test_data']['category_with_subcategory']
    result = processor._get_attribute_data(test_data, FyleAttributeTypeEnum.CATEGORY)
...
def test_get_attribute_data_category_no_subcategory(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    test_data = data['webhook_test_data']['category_without_subcategory']
    result = processor._get_attribute_data(test_data, FyleAttributeTypeEnum.CATEGORY)
...
def test_process_webhook_unsupported_resource_type(db):
    ...
    workspace_id = 1
    processor = WebhookAttributeProcessor(workspace_id)
    webhook_body = data['webhook_payloads']['unsupported_resource']
